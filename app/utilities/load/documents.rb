...
#             Remove any singleton backslashes in abstracts (only
#             place the problem has been present).
              ...
              load.into_lna(hash) if (!@unknownPersons[hash[:netid]])
            rescue StandardError => e
...
#   Creates the document described by the given hash. If a netid is
#   given this document is associated with the corresponding
#   person. At this point only ingesting documents by netid is
#   supported.
#
...
#   Creates the document described and associates it with the
...
#   @param [Hash] hash document properties
    def into_lna_by_netid!(netid, hash)
      ...
      unless person = find_person_by_netid(netid)
        ...
        @unknownPersons[netid] = 1
        return
      end

      collection = person.collections.first

#     If there's an elements id, check to see if there's already a
#     document with that id.  If there is, don't add it again.
